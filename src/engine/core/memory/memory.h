...
#define ucNewArray(type, count, ...) Unicorn::memory::ConstructAndAllocate<type>(count, __VA_ARGS__)
#define ucNew(type, ...) ucNewArray(type, 1, __VA_ARGS__)

#define ucDeleteArray(ptr, count) Unicorn::memory::DestroyAndDeallocate(ptr, count); ptr = nullptr
